...
**源码版本：1.6.0 · 已验证 Pi：0.85.1 · Windows 10/11 x64**
...
| 操作 | 方法 |
| --- | --- |
| 新对话 | 点击“新对话”直接使用默认目录；“选择目录新建…”用于单次指定项目 |
| 默认目录 | 初始为 `%USERPROFILE%\Pi Harbor\Workspace`，可点“默认目录…”修改并保存 |
...
已提供 Windows CI 和版本标签发布工作流。推送与项目版本一致的标签（当前源码为 `v1.6.0`）后，自动测试、打包并创建带 **安装 EXE、便携 ZIP、SHA256SUMS** 的 Release 草稿；维护者检查后点击 Publish release，用户即可下载。
