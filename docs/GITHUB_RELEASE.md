...
2. 确认 `src/PIHarness.App/PIHarness.App.csproj` 中 Version、AssemblyVersion、FileVersion、InformationalVersion 一致，README 与 CHANGELOG 更新。当前源码为 `1.6.0`。
3. 本机运行完整测试、Release UI 验收和安装/卸载验收；公开截图只使用 `docs/images/` 的脱敏图。
4. 提交并推送代码后，创建与版本一致的标签：

```powershell
git tag -a v1.6.0 -m "Pi Harbor 1.6.0"
